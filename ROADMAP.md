...
> **Status: Done (Session 21, commit 488b23f).** Manifest and service worker
> now generated by `vite-plugin-pwa`. See SPEC.md "PWA (Session 21)" for the
> implementation, including the `orientation: 'any'` deviation from the
> literal proposal below (landscape is locked imperatively during gameplay
> only, not manifest-wide) and the full caching strategy.
...
> **Status: Done (Session 21, commit 488b23f), implemented differently than
> proposed below.** This section's own text below weighs queuing vs. blocking
> and leans toward queuing; the actual decision (made explicitly, not
> discovered mid-implementation) was to **block, not queue**. Offline
> submission is a dead end: no name input, no POST, a plain message.
> Rationale: `qualifies()` needs a live board to know whether a score would
> even rank, so offline there's no way to show a truthful qualification
> prompt; queuing would mean either prompting on every offline round (most
> non-qualifying) or deferring the qualify check to flush time (submitting
> something the player was never shown as qualifying). See SPEC.md "Offline
> Behavior" for the implemented behavior. A real bug in this same reconnect
> path (a stale late-arriving response could clobber a successful retry) was
> found on-device and fixed in Session 22; see the note at the end of this
> file.
...
> **Status: Done (Session 21, commit 488b23f).** `/api/*` is NetworkOnly in
> the Workbox config, always. The narrow amendment below (a labelled
> stale-while-revalidate for the overlay's display-only fetch) was **not**
> taken; both the End screen's qualification fetch and the overlay's display
> fetch are network-only. Simpler, and the amendment was explicitly optional
> in the original note. See SPEC.md "PWA (Session 21)".
...
> **Status: Done (Session 21, commit 488b23f).** `index.html` is NetworkFirst,
> implemented via a Workbox `runtimeCaching` rule rather than the "network-
> first with short timeout falling back to cache" phrasing below, to the same
> effect. One thing this proposal didn't anticipate: `vite-plugin-pwa`
> defaults to its own `navigateFallback: 'index.html'`, which registers a
> precache-bound route ahead of any custom rule and would have silently
> defeated the NetworkFirst rule even with it correctly configured. Required
> explicitly setting `navigateFallback: null`. Caught by inspecting the
> generated `sw.js`, not by assumption. See SPEC.md "PWA (Session 21)".
...
> **Status: Done (Session 21, commit 488b23f), with a judgment call flagged
> for your device check.** The four MP3s are **not** in the default precache
> (which has no Range support); they're routed through a dedicated
> `CacheFirst` entry with `rangeRequests: true` (Workbox's declarative
> shortcut for `workbox-range-requests`), which is the "standard fix" this
> section already names. Trade-off versus "precache all four" as literally
> proposed below: the ambient loop still warms almost immediately (already
> preloaded on mount by `useSound.js`), but the three one-shot SFX only cache
> on first trigger, not at install. Still needs your on-device confirmation:
> play all four sounds offline, including on a fresh install before any of
> the three one-shots have ever fired once online. See SPEC.md "PWA (Session
> 21)".
...
| Background Sync (**A6**) | Absent. Moot: A6 was implemented as block-not-queue (Session 21), so there is no queue/flush logic to test. | N/A. |
...
# Session 22: Device-discovered bugs (fixed)

Not part of the original Session 19 audit's numbered findings (B1-B14,
O1-O28, A1-A15). These three were found during real-device verification of
Phase 1 (the PWA work above), fixed the same session, and are documented
here so the audit trail stays complete. Commit 3efc101 on `pwa`, merged to
`main` in the same state. **Status: Done.**

Note the naming collision to avoid confusion: "Bug 1/2/3" below are Session
22's own numbering, unrelated to B1-B14 above (e.g. Session 22's Bug 1 is
not the same thing as Finding B1, the End-screen clipping bug, which is
still open).

- **Bug 1, personal best read/written against the wrong difficulty.**
  `endGame()` in `App.jsx` is called from a `useCallback` (`onFrameUpdate`)
  that is memoized exactly once for the component's entire lifetime, so it
  permanently closed over whatever `difficulty` was on App's first render.
  Not display-only: this also corrupted which difficulty's PB got written to
  on a genuine new best. Fixed by locking the round's difficulty into a ref
  at `startGame()`, mirroring the existing `fleeSettingsRef` pattern.
- **Bug 2, "Couldn't load scores" could persist after a healthy reconnect.**
  A race in the Phase 1 reconnect handler: an offline request can take a long
  time to actually reject (not fail instantly), so a late rejection could
  land after a successful retry and clobber it. Fixed with a request-id
  guard in `loadPreview()`.
- **Bug 3, fish could escape world bounds.** Confirmed the mechanism
  described in B3/O-adjacent reasoning about edge repulsion: flee is a
  constant-strength force within its radius while edge repulsion only ramps
  up near the wall, and the old weight (3.0) never exceeded the worst-case
  flee weight (4.0, Hardcore). Retuned `EDGE_REPULSION_WEIGHT` 3.0 to 6.0 and
  `EDGE_REPULSION_RADIUS` 120 to 140 (verified by simulation, not guesswork),
  backstopped with a hard positional clamp in `updateFish()` so this can't
  regress silently even if tuning drifts again.
